            ...
            <div class="card-body">
                <form action="/update_nilai/{{ $nilai->id_nilai }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="nilai" id="nilai" class="form-control" min="0"
                            max="100" value="{{ $nilai->nilai }}" required>
                    </div>

                    <div class="mb-3">
                        <label for="berkas" class="form-label">Berkas (PDF/DOC, dll.) - Biarkan kosong untuk menjaga
                            berkas lama</label>

                        @if (!empty($nilai->berkas))
                            <div class="mb-2">
                                <a href="{{ asset('dokumen/' . $nilai->berkas) }}" target="_blank">
                                    {{ $nilai->berkas }}
                                </a>
                            </div>
                        @endif
                        ...
                        <div class="form-text">Opsional — unggah file baru untuk mengganti berkas lama.</div>
                    </div>
